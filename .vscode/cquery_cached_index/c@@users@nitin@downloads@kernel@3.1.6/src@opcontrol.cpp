...
void opcontrol() {
  ...
  std::vector <pros::Motor> driveMotors={Drive_FR,Drive_FL,Drive_BR,Drive_BL};
	...
	while (true) {
		...
		{
			Roller.move_velocity(270);
			Indexer.move_velocity(270);
		}
		...
		if(master.get_digital(DIGITAL_A)) //Lift up
		{
			Lift.move_velocity(270);
		}
		else if(master.get_digital(DIGITAL_B)) //Lift down
		{
			Lift.move_velocity(-270);
		}

		if(master.get_digital(DIGITAL_R1)) //Roller intake with indexer preventing ball going to flywheel
		{
			Roller.move_velocity(270);
			Indexer.move_velocity(-270);
		}
